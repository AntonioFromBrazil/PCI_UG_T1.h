...
void loop(){
  ...
  if ((graus==555)&&(x.xsteps[0]==0)&&(!client)){
    x.xms=500;while(x.xms>0){}
    if (sent=='D'){x.runStep(0, 2048, vel, true);}else{x.runStep(0, 2048, vel, false);}
  }
  if (client) {
      ...
      if (client.available()) {
        ...
        if (c == '\n') {
          if (currentLine.length() == 0) {
            ...
            if (header.indexOf("GET /VV3") >= 0) {graus=555;}
            ...
            if (graus==555){copyhtml.replace("#VV3", "#0099FF");}else{copyhtml.replace("#VV3", "#99CCFF");}
            ...
            //starta o motor
            while(x.xsteps[0]>0){}
            if ((graus==999)&&(graus!=555)){nsteps=2048000;}
            if ((graus!=999)&&(graus!=555)){nsteps=map(graus,0,360,0,2048);}
